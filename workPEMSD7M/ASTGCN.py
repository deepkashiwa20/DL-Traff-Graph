...
class ASTGCN(nn.Module):
    def __init__(self, DEVICE, cheb_polynomials, nb_block=2, in_channels=1, K=3, nb_chev_filter=64, nb_time_filter=64,
                 time_strides=2, num_for_predict=TIMESTEP_OUT, len_input=TIMESTEP_OUT * (WEEK + DAY + HOUR),
...
#     adj_mx, _ = get_adjacency_matrix(distance, N_NODE)
    ...
    model = ASTGCN(device, cheb_polynomials = cheb_polynomials,nb_block =2,
                   ...
                   num_for_predict = TIMESTEP_OUT, len_input = TIMESTEP_OUT*(WEEK+DAY+HOUR), num_of_vertices = N_NODE).to(device)
